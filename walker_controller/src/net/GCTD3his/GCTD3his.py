...
class GCTD3his(object):
    ...
    def save_model(self, filename, agent):
        ...
        torch.save(self.critic.state_dict(), save_dir + filename + "_critic.pt")
        torch.save(self.critic_optimizer.state_dict(), save_dir+ filename + "_critic_optimizer.pt")
        
        torch.save(self.actor.state_dict(), save_dir + filename + "_actor.pt")
        torch.save(self.actor_optimizer.state_dict(), save_dir + filename + "_actor_optimizer.pt")
        print("Saved!!!")
        
    def load_model(self, filename, agent):
        load_dir = f"weights/{agent}/"
        self.critic.load_state_dict(torch.load(load_dir + filename + "_critic.pt"))
        self.critic_optimizer.load_state_dict(torch.load(load_dir + filename + "_critic_optimizer.pt"))
        self.critic_target = copy.deepcopy(self.critic)

        self.actor.load_state_dict(torch.load(load_dir + filename + "_actor.pt"))
        self.actor_optimizer.load_state_dict(torch.load(load_dir + filename + "_actor_optimizer.pt"))
        self.actor_target = copy.deepcopy(self.actor)
